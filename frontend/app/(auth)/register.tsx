    ...
    <SafeAreaView className="flex-1 bg-black">
      <KeyboardAvoidingView className="flex-1" behavior={Platform.OS === "ios" ? "padding" : "height"}>
        <View className="flex-1 justify-center px-7">
          <View className="mb-8">
            <Text className="text-3xl font-black text-white tracking-tight">アカウント作成</Text>
            <Text className="text-gray-300 text-sm mt-2">今日から努力を記録しよう 🔥</Text>
          </View>
          ...
          <View className="gap-4 mb-6">
            {[
              { name: "email" as const, label: "メールアドレス", placeholder: "example@email.com", secure: false, keyboard: "email-address" as const },
              { name: "password" as const, label: "パスワード", placeholder: "8文字以上", secure: true },
              { name: "confirmPassword" as const, label: "パスワード（確認）", placeholder: "もう一度入力", secure: true },
            ].map((f) => (
              <View key={f.name}>
          ...
          >
            {loading
              ...
              : <Text className="text-white font-bold text-base">登録する</Text>
            }
          </TouchableOpacity>

          <TouchableOpacity className="items-center" onPress={() => router.back()}>
            <Text className="text-gray-300 text-sm">
              すでにアカウントをお持ちの方は <Text className="text-white font-bold">ログイン</Text>
            </Text>
          </TouchableOpacity>
        </View>
